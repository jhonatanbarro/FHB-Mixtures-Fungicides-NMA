...
```{r}
dat_sev <- fhb2 %>%
  select(1:12, 21, 25:32, 35:38) %>%
  group_by(trial, year, location, state, cultivar, reaction, n, AI, n_spray, yld_check, sev_check, mean_sev, publication, author, cons_group, AI2) %>%
  summarise(
    sev = mean(sev),
    V_sev = mean(V_sev)) %>% 
  filter(sev != "NA") %>% 
